...
export default function Home() {
  ...
  return (
    <div className="grid place-items-center mt-10">
      <button className="bg-[white] text-black p-5" onClick={handleClick}>
        Do Server Action
      </button>

      <button className="mt-10 bg-[white] text-black p-5" onClick={fetchDate}>
        Get users data
      </button>
      ...
      <table className="mt-10 border-collapse border border-gray-800">
        ...
        <tbody>
          {userList.map((user) => (
            <tr key={user.id}>
              <td className="border border-gray-800 py-1 px-3">
                <Link href={`posts/${user.id}`}>{user.id}</Link>
              </td>
